...
function ${luaNamespace!""}${bean.name}:write(buf)
    ...
    if ${field.name}_len > 0 then
        ...
        <#elseif field.classType="long">
            packLong(buf,self.${field.name}[i])
...
function ${luaNamespace!""}${bean.name}:read(buf)
    ...
    --${field.explain}
        ...
        if ${field.name}_len > 0 then
            for i=1,${field.name}_len do
            <#if field.classType="boolean">
            self.${field.name}[i] = buf:readBool()
            <#elseif field.classType="byte">
            self.${field.name}[i] = buf:readRawByte()
            <#elseif field.classType="short">
            self.${field.name}[i] = buf:readShort()
            <#elseif field.classType="int">
            self.${field.name}[i] = buf:readInt()
            <#elseif field.classType="float">
            self.${field.name}[i] = buf:readFloat()
            <#elseif field.classType="double">
            self.${field.name}[i] = buf:readDouble()
            <#elseif field.classType="long">
            self.${field.name}[i] = buf:readLong()
            <#elseif field.classType="String">
            local ${field.name}_len = buf:readUInt()
            if ${field.name}_len  > 0 then
                self.${field.name}[i] = buf:readStringBytes(${field.name}_len )
            else
                self.${field.name}[i] = ""
            end
            <#else>
                ...
                self.${field.name}[i] = _${field.classType?uncap_first}
            </#if>
            end
        end
        <#else ><#--不是list-->
